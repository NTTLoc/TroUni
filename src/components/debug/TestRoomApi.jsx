...
const TestRoomApi = () => {
  ...
  const fetchRooms = async () => {
    ...
    try {
      ...
      console.log("🧪 TestRoomApi: Response.data keys:", Object.keys(response.data || {}));
      
      const roomsData = response.data?.content || [];
      console.log("🧪 TestRoomApi: Rooms data:", roomsData);
      console.log("🧪 TestRoomApi: Rooms data length:", roomsData.length ? roomsData.length : "NO LENGTH");
  ...
  if (error) {
    return (
      <div style={{ padding: "20px", textAlign: "center" }}>
        <Card style={{ margin: "10px", background: "#fff2f0", borderColor: "#ff4d4f" }}>
          <Title level={4} style={{ color: "#ff4d4f" }}>❌ Lỗi API</Title>
          <Text>{error}</Text>
    ...
    <div style={{ padding: "20px" }}>
      ...
      <Card style={{ marginBottom: "20px" }}>
        ...
        <Space direction="vertical">
          <Text><strong>Tổng số phòng:</strong> {stats.total}</Text>
          <Text><strong>Trang hiện tại:</strong> {stats.currentPage + 1}/{stats.totalPages}</Text>
          <Text><strong>Số phòng/trang:</strong> {stats.pageSize}</Text>
          <Text><strong>Số phòng đang hiển thị:</strong> {rooms.length}</Text>
        </Space>
        <div style={{ marginTop: "10px" }}>
          ...
          {rooms.map((room, index) => (
            ...
            >
              <Space direction="vertical" style={{ width: "100%" }}>
                ...
                <div><strong>Tiêu đề:</strong> {room.title || "Không có"}</div>
                <div><strong>Mô tả:</strong> {room.description || "Không có"}</div>
                <div><strong>Địa chỉ:</strong> {room.streetAddress || "Không có"}</div>
                <div><strong>Thành phố:</strong> {room.city || "Không có"}</div>
                <div><strong>Quận:</strong> {room.district || "Không có"}</div>
                <div><strong>Phường:</strong> {room.ward || "Không có"}</div>
                <div><strong>Giá:</strong> {room.pricePerMonth ? `${room.pricePerMonth.toLocaleString()} VNĐ` : "Không có"}</div>
                <div><strong>Diện tích:</strong> {room.areaSqm ? `${room.areaSqm} m²` : "Không có"}</div>
                <div><strong>Loại phòng:</strong> {room.roomType || "Không có"}</div>
                <div><strong>Trạng thái:</strong> {room.status || "Không có"}</div>
                <div><strong>Ngày tạo:</strong> {room.createdAt ? new Date(room.createdAt).toLocaleString() : "Không có"}</div>
                <div><strong>Số ảnh:</strong> {room.images ? room.images.length : "0"}</div>
              </Space>
            </Card>
          ))}
        </div>

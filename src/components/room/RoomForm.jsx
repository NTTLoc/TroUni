    ...
    try {
      ...
      if (isEditMode) {
        ...
        console.log(
          ...
          imageList.length > 0 && (result?.data?.id || result?.id)
        );
        ...
        if (imageList.length > 0 && roomId) {
          try {
            ...
            for (const img of imageList) {
              ...
              if (img.originFileObj) {
                try {
                  ...
                  message.error(
                    `Lỗi xử lý ảnh ${img.originFileObj.name}: ${error.message}`
                  );
                }
              }
            }
            ...
            if (imageUrls.length > 0) {
              ...
              message.success(
                `Lưu thành công ${imageUrls.length} ảnh vào database!`
              );
            } else {
            ...
            message.warning(
              "Phòng trọ đã tạo nhưng upload ảnh thất bại. Vui lòng thử lại sau."
            );
          }
        }
      }

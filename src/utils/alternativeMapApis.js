...
export const searchWithGooglePlaces = async (query) => {
  try {
    ...
    const response = await fetch(
      `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(query)}&key=YOUR_API_KEY&region=vn&language=vi`
    );
    ...
    return data.results.map(place => ({
      ...
      address: {
        ...
        district: place.address_components?.find(c => c.types.includes('administrative_area_level_2'))?.long_name || '',
...
export const searchWithMapBox = async (query) => {
  try {
    const response = await fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(query)}.json?access_token=YOUR_MAPBOX_TOKEN&country=VN&language=vi&limit=5`
    );
    ...
    return data.features.map(feature => ({
      ...
      address: {
        ...
        district: feature.context?.find(c => c.id.startsWith('district'))?.text || '',
...
export const searchWithHERE = async (query) => {
  try {
    const response = await fetch(
      `https://geocode.search.hereapi.com/v1/geocode?q=${encodeURIComponent(query)}&apikey=YOUR_HERE_API_KEY&in=countryCode:VN&limit=5&lang=vi`
    );
  ...
  const searchMethods = [
    ...
    async () => {
      const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=5&addressdetails=1&accept-language=vi&countrycodes=vn`
      )}`;
